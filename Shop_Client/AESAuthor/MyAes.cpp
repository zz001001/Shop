...
void MyAes::SubBytes(Mybyte mtx[4*4])
{
	for(int i=0; i<16; ++i)
	{
 ...
 *  行变换 - 按字节循环移位
 */
void MyAes::ShiftRows(Mybyte mtx[4*4])
{
	// 第二行循环左移一位
	Mybyte temp = mtx[4];
	for(int i=0; i<3; ++i)
 ...
 *  列变换
 */
void MyAes::MixColumns(Mybyte mtx[4*4])
{
	Mybyte arr[4];
	for(int i=0; i<4; ++i)
	{
 ...
 */
void MyAes::AddRoundKey(Mybyte mtx[4*4], word k[4])
{
	for(int i=0; i<4; ++i)
	{
		word k1 = k[i] >> 24;
		...
		word k4 = (k[i] << 24) >> 24;
		// 异或运算
		mtx[i] = mtx[i] ^ Mybyte(k1.to_ulong());
		mtx[i+4] = mtx[i+4] ^ Mybyte(k2.to_ulong());
		mtx[i+8] = mtx[i+8] ^ Mybyte(k3.to_ulong());
		mtx[i+12] = mtx[i+12] ^ Mybyte(k4.to_ulong());
	}
}
 ...
 *  逆S盒变换
 */
void MyAes::InvSubBytes(Mybyte mtx[4*4])
{
	for(int i=0; i<16; ++i)
	{
		int row = mtx[i][7]*8 + mtx[i][6]*4 + mtx[i][5]*2 + mtx[i][4];
 ...
 *  逆行变换 - 以字节为单位循环右移
 */
void MyAes::InvShiftRows(Mybyte mtx[4*4])
{
	// 第二行循环右移一位
	Mybyte temp = mtx[7];
	for(int i=3; i>0; --i)
...
void MyAes::InvMixColumns(Mybyte mtx[4*4])
{
	Mybyte arr[4];
	for(int i=0; i<4; ++i)
	{
...
/******************************下面是密钥扩展部分***********************/
/**
 * 将4个 Mybyte 转换为一个 word.
 */
word MyAes::Word(Mybyte& k1, Mybyte& k2, Mybyte& k3, Mybyte& k4)
{
 ...
 */ 
void MyAes::KeyExpansion(Mybyte key[4*Nk], word w[4*(Nr+1)])
{
	word temp;
	int i = 0;
	// w[]的前4个就是输入的key
 ...
 *  加密
 */
void MyAes::encrypt(Mybyte in[4*4], word w[4*(Nr+1)])
{
	word key[4];
	for(int i=0; i<4; ++i)
		key[i] = w[i];
 ...
 *  解密
 */
void MyAes::decrypt(Mybyte in[4*4], word w[4*(Nr+1)])
{
	word key[4];
	for(int i=0; i<4; ++i)
		key[i] = w[4*Nr+i];
